...
RegisterCommand("testprogg", function()
    SendNUIMessage({
        action = "startAction",
        timer = 15000,
        content = "Benzin dolduruluyor...",
        icon = Config["icons"]["default"],
        escapeKey = Config.escapeKey.label
    })
    inprogress = true
end)
...
local function StartProgress(action, onStart, onTick, onFinish)
    ...
    if (not isPlayerDead or action.useWhileDead) and not inprogress then
        inprogress = true
        ...
        SendNUIMessage({
            ...
            icon = Config["icons"][action.icon] or Config["icons"]["default"],
            escapeKey = Config.escapeKey.label
        })
        StartActions()
        CreateThread(function()
            ...
            while inprogress do
                ...
                if IsControlJustPressed(0, Config.escapeKey.id) and action.canCancel then
